...
public class MainController {
    ...
    public ResponseEntity<String> issueIOU(HttpServletRequest request) throws IllegalArgumentException {
        ...
        String key = request.getParameter("iouValue");
        ...
        try {
            ...
            SignedTransaction result = proxy.startTrackedFlowDynamic(IOUFlow.Initiator.class, key,otherParty).getReturnValue().get();
            // Return the response.
    ...
    public ResponseEntity<List<StateAndRef<IOUState>>> getMyIOUs() {
        List<StateAndRef<IOUState>> myious = proxy.vaultQuery(IOUState.class).getStates().stream().filter(
                it -> it.getState().getData().getCreator().equals(proxy.nodeInfo().getLegalIdentities().get(0))).collect(Collectors.toList());
