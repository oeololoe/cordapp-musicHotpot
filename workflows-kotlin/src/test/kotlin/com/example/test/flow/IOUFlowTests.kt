...
class IOUFlowTests {
    ...
    fun `flow rejects invalid IOUs`() {
        val flow = IOUFlow.Initiator("", b.info.singleIdentity())
    ...
    fun `recorded transaction has no inputs and a single output, the input IOU`() {
        ...
        for (node in listOf(a, b)) {
            ...
            assertEquals(recordedState.key, iouValue)
            assertEquals(recordedState.creator, a.info.singleIdentity())
            assertEquals(recordedState.follower, b.info.singleIdentity())
        }
    }
